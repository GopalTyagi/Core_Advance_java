...
public class UserModel {
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getConfirmPassword() != null && bean.getConfirmPassword().length() > 0) {
				sql.append(" AND PASSWORD like '" + bean.getConfirmPassword() + "%'");
			}
	 ...
	 * newPassword) throws RecordNotFoundException, ApplicationException {
	 * 
	 * log.debug("model changePassword Started"); boolean flag = false; UserBean
	 * beanExist = null;
	 * 
	 * beanExist = findByPK(id); if (beanExist != null &&
	 * beanExist.getPassword().equals(oldPassword)) {
	 * beanExist.setPassword(newPassword); try { update(beanExist); } catch
	 ...
	 *//**
		 ...
		 *//*
			 ...
			 * RecordNotFoundException { UserBean userData = findByLogin(login); boolean
			 * flag = false;
			 * 
			 * if (userData == null) { throw new
			 * RecordNotFoundException("Email ID does not exists !");
			 * 
			 * }
			 * 
			 * HashMap<String, String> map = new HashMap<String, String>(); map.put("login",
